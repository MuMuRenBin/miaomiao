        ...
        <div class="city_list">
            ...
            <div class="city_sort" ref="city_sort">
                <div v-for="(item, index) in cityList" :key="index">
                    <h2>{{item.index}}</h2>
                    <ul>
                        <li v-for="(itemlist, index) in item.list" :key="index">{{itemlist.nm}}</li>
                    </ul>
                </div>
...
export default {
    ...
    mounted() {
        // this.axios.get('/api/movie/citys').then((result) => {
        //     console.log(result);
        // }).catch((err) => {
        //     console.log(err);
        // });
...
<style scoped>
    ...
    .city_body{margin-top:95px;display: flex;width: 100%;position: fixed; top: 0; bottom: 0;}
    ...
    .city_body .city_index{width:20px;display: flex;flex-direction: column;text-align:center;border-left: 1px solid #e6e6e6;}
</style>